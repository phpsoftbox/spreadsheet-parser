...
final class XlsxWriter
{
    ...
    private function worksheetXml(array $rows): string
    {
        ...
        foreach ($rows as $rowIndex => $row) {
            ...
            foreach ($row as $colIndex => $cell) {
                if ($cell === null || ($cell instanceof SpreadsheetCell && $cell->value === null)) {
                    continue;
                }
    ...
    private function cellXml(string $ref, string|int|float|bool|SpreadsheetCell $cell): string
    {
        if ($cell instanceof SpreadsheetCell) {
            return $this->typedCellXml($ref, $cell);
        }

        return $this->autoCellXml($ref, $cell);
    }

    private function typedCellXml(string $ref, SpreadsheetCell $cell): string
    {
        if ($cell->value === null) {
            return '';
        }
        ...
        return match ($cell->type) {
            SpreadsheetCellType::Auto   => $this->autoCellXml($ref, $cell->value),
            SpreadsheetCellType::Text   => $this->textCellXml($ref, (string) $cell->value),
            SpreadsheetCellType::Number => is_int($cell->value) || is_float($cell->value)
                ? $this->numberCellXml($ref, $cell->value)
                : '',
            SpreadsheetCellType::Boolean => $this->booleanCellXml($ref, (bool) $cell->value),
        };
    }

    private function autoCellXml(string $ref, string|int|float|bool $cell): string
    {
        if (is_bool($cell)) {
            return $this->booleanCellXml($ref, $cell);
        }

        if (is_int($cell) || is_float($cell)) {
            return $this->numberCellXml($ref, $cell);
        }

        return $this->textCellXml($ref, (string) $cell);
    }

    private function booleanCellXml(string $ref, bool $cell): string
    {
        return '<c r="' . $ref . '" t="b"><v>' . ($cell ? '1' : '0') . '</v></c>';
    }

    private function numberCellXml(string $ref, int|float $cell): string
    {
        return '<c r="' . $ref . '"><v>' . (string) $cell . '</v></c>';
    }

    private function textCellXml(string $ref, string $cell): string
    {
