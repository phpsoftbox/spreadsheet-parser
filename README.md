...
`SpreadsheetParser` — компонент импорта и экспорта табличных файлов `CSV/XLSX` для PhpSoftBox.
...
- экспорт CSV/XLSX через `SpreadsheetWriter`;
- явный текстовый тип экспортируемых ячеек через `SpreadsheetCell::text(...)`;
