...
final class XlsxTestFileFactory
{
    ...
    public static function create(array $sheets, ?string $stylesXml = null): string
    {
        ...
        foreach ($sheets as $sheet) {
            $xmlSheets[] = [
                'name' => $sheet['name'],
                'xml'  => self::worksheetXml($sheet['rows']),
            ];
        }

        return self::createFromWorksheetXmls($xmlSheets, $stylesXml, null);
    }

    public static function createFromWorksheetXml(
        string $sheetName,
        string $worksheetXml,
        ?string $stylesXml = null,
        ?string $sharedStringsXml = null,
    ): string {
        return self::createFromWorksheetXmls([
            [
                'name' => $sheetName,
                'xml'  => $worksheetXml,
            ],
        ], $stylesXml, $sharedStringsXml);
    }

    /**
     * @param list<array{name: string, xml: string}> $sheets
     */
    private static function createFromWorksheetXmls(
        array $sheets,
        ?string $stylesXml = null,
        ?string $sharedStringsXml = null,
    ): string {
        ...
        if ($sharedStringsXml !== null) {
            ...
            $sharedStringsContentType = '<Override PartName="/xl/sharedStrings.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sharedStrings+xml"/>';
        }
