...
final class SpreadsheetParserTest extends TestCase
{
    ...
    #[Test]
    /**
     * Проверяет, что XLSX-ячейки без r читаются по порядку внутри строки.
     */
    public function importsXlsxCellsWithoutReferencesByOrder(): void
    {
        $file = $this->xlsxWithWorksheetXml(
        ...
        self::assertFalse($result->hasErrors());
        self::assertSame(['barcode', 'qty'], $result->headers);
        self::assertSame('2042905287108', $result->rows[0]['barcode']);
        self::assertSame(50, $result->rows[0]['qty']);
    }

    #[Test]
    /**
     * Проверяет, что XLSX-ячейка без r после B попадает в C и не перетирает B.
     */
    public function importsMixedXlsxCellsUsingNextFallbackColumn(): void
    {
        $file = $this->xlsxWithWorksheetXml(
            '<?xml version="1.0" encoding="UTF-8"?>'
            . '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>'
            . '<row r="1">'
            . '<c r="B1" t="inlineStr"><is><t>barcode</t></is></c>'
            . '<c t="inlineStr"><is><t>qty</t></is></c>'
            . '</row>'
            . '<row r="2">'
            . '<c r="B2" t="inlineStr"><is><t>2042905287108</t></is></c>'
            . '<c><v>50</v></c>'
            . '</row>'
            . '</sheetData></worksheet>',
        );
        ...
        self::assertFalse($result->hasErrors());
        self::assertSame(['column_1', 'barcode', 'qty'], $result->headers);
        self::assertSame('2042905287108', $result->rows[0]['barcode']);
        self::assertSame(50, $result->rows[0]['qty']);
    }
    ...
    #[Test]
    /**
     * Проверяет, что невалидный r обрабатывается как отсутствующий r.
     */
    public function importsXlsxCellsWithInvalidReferencesByOrder(): void
    {
        $file = $this->xlsxWithWorksheetXml(
            '<?xml version="1.0" encoding="UTF-8"?>'
            . '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>'
            . '<row r="1">'
            . '<c r="invalid" t="inlineStr"><is><t>barcode</t></is></c>'
            . '<c t="inlineStr"><is><t>qty</t></is></c>'
            . '</row>'
            . '<row r="2">'
            . '<c r="invalid" t="inlineStr"><is><t>2042905287108</t></is></c>'
            . '<c><v>50</v></c>'
            . '</row>'
            . '</sheetData></worksheet>',
        );
    ...
    private function xlsxWithWorksheetXml(string $worksheetXml, ?string $sharedStringsXml = null): string
    {
